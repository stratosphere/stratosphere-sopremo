...
public class Graph<Node> implements Iterable<Graph<Node>.NodePath> {
	...
	public class NodePath implements Iterable<NodePath> {
		...
		public Iterator<NodePath> iterator() {
			return Iterators.transform(Graph.this.navigator.getConnectedNodes(this.node).iterator(),
				new Function<Node, NodePath>() {
					int outIndex = 0;

					@Override
					public NodePath apply(final Node inputObject) {
