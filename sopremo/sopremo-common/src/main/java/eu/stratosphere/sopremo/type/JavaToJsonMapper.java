...
public class JavaToJsonMapper {
	...
	private final HashMap<Class<? extends Object>, Constructor<? extends IJsonNode>> typeDict =
		new HashMap<Class<? extends Object>, Constructor<? extends IJsonNode>>();
	...
	public JavaToJsonMapper() {
		try {
			...
			this.typeDict.put(CharSequence.class, TextNode.class.getConstructor(CharSequence.class));
	...
	public Class<? extends IJsonNode> classToJsonType(final Class<?> javaClass) {
		...
		if (CharSequence.class.isAssignableFrom(javaClass) || javaClass.isEnum())
		...
		final Class<?> boxingClass = Primitives.wrap(javaClass);
		if (boxingClass != null) {
			Constructor<? extends IJsonNode> primitiveConstructor = this.typeDict.get(boxingClass);
			if (primitiveConstructor != null)
				return primitiveConstructor.getDeclaringClass();
		}
	...
	public IJsonNode valueToTree(final Object value) {
		...
		if (valueClass.isArray()) {
			final ArrayNode<IJsonNode> arrayNode = new ArrayNode<IJsonNode>();
